...
void writeSE0164(uint16 regaddr, uint8 regdata) {
	...
	ExternalRAM_enable();	//切换到片外RAM
	SE0164_CS = 0;			//片选生效
	XBYTE[SE0164_GLOBAL_WLOCK_REG] = 0xAA;		//关闭写保护
	XBYTE[regaddr] = regdata;
	XBYTE[SE0164_GLOBAL_WLOCK_REG] = 0x00;		//打开写保护
	SE0164_CS = 1;
	InternalRAM_enable();
	ES = 1;
}
 ...
 */
void softReset(void) {
	writeSE0164(SE0164_GLOBAL_RSTLOOP1_REG, 0x01);
    delay_ms(5*12);
	writeSE0164(SE0164_GLOBAL_RSTLOOP1_REG, 0x00);
    delay_ms(5*12);
}
...
bool setDefault(void) {
	char i = 0;
	if( !writeSE0164Verify(SE0164_LCAS_TVCG_CFG, 0x03, 0xff) ) {	//发送方向 LCAS VCAT使能
		return false;
	}
	if( !writeSE0164Verify(SE0164_LCAS_RVCG_CFG, 0x03, 0xff) ) {	//接收方向 LCAS VCAT使能
		return false;
	}

	for (i = 0; i < 4; ++i) {
		//支路1-4使能
		if( !writeSE0164Verify(SE0164_LCAS_TTRIB_USE_SQ(i), 0x1f, 0x1f) ) {
			return false;
		}
		if( !writeSE0164Verify(SE0164_LCAS_RTRIB_USE_SQ(i), 0x1f, 0x1f) ) {
			return false;
		}
	}
	return true;
...
int getE1Alarm(uint8 portN) {
	if(portN >= SE0164_E1_MAXNUM ) {
		return -1;
	}
	return (readSE0164(SE0164_E1_ALARM2(portN))<<7) | readSE0164(SE0164_E1_ALARM1(portN));
}
...
void writeTCMFData(uint8 sn, uint8 b) {
	writeSE0164(SE0164_GFP_TCMF_DAT_INDEX, sn);
	writeSE0164(SE0164_GFP_TCMF_DAT, b);
}

void setTCMFLength(uint8 len) {
	writeSE0164(SE0164_GFP_TCMF_DAT_LEN, len);
}
bool ifTCMFOver(void) {
	return (readSE0164(SE0164_GFP_TCMF_CTRL) & (1<<6)) != 0;
}
void startTCMF(void) {
	writeSE0164(SE0164_GFP_TCMF_CTRL, (1<<7));
}

bool ifRCMFReady(void) {
	uint8 v = readSE0164(SE0164_GFP_RCMF_READY);
	return (v & 1) == 1;
}
uint8 getRCMFLength(void) {
	return readSE0164(SE0164_GFP_RCMF_DAT_LEN);
}
uint8 readRCMFData(uint8 sn) {
	writeSE0164(SE0164_GFP_RCMF_DAT_INDEX, sn);
	return readSE0164(SE0164_GFP_RCMF_DAT);
}
